...
import { tasksContainer } from "../database/cosmosClient";
...
export const getTasks = async ({
  ...
  const { resources: countResources } = await tasksContainer.items
    .query(countQuery)
    .fetchAll();
...
export const getTaskById = async (taskId: string, partitionKey: string) => {
  try {
    const { resource } = await tasksContainer.item(taskId, partitionKey).read();
...
export const deleteTask = async (taskId: string, partitionKey: string) => {
  try {
    await tasksContainer.item(taskId, partitionKey).delete();
  } catch (err: any) {
    if (err.code === 404 || err.statusCode === 404) {
      throw new AppError("Task not found", 404);
    }
...
export const bulkDeleteTasks = async (
  ...
  try {
    const deletePromises = taskIds.map((id) =>
      tasksContainer.item(id, partitionKey).delete(),
    );
